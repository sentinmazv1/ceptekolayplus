...
export function ContractDocument({ customer }: ContractDocumentProps) {
        ...
        if (customer.satilan_urunler) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                productInfo = {
                    ...
                    price: item.fiyat || customer.talep_edilen_tutar || 0
                };
            }
        }
        if (!productInfo.name && customer.talep_edilen_urun) {
            productInfo.name = customer.talep_edilen_urun;
            productInfo.price = customer.talep_edilen_tutar || 0;
        }
    } catch (e) { console.error(e); }

    // Calculate Installments (Mocking logic or just empty table for manual fill)
    // As automated installment logic isn't strictly defined yet, we'll leave strict values empty or generic.
    const priceStr = productInfo.price ? new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(productInfo.price) : '...................... TL';
    ...
    return (
        <div className="bg-white text-black p-8 max-w-[210mm] mx-auto text-[10px] leading-tight font-serif" style={{ minHeight: '297mm' }}>
                    ...
                    <tr>
                        <td className="border border-black p-2 align-top h-40">
                            <div className="grid grid-cols-[80px_1fr] gap-1">
                                ...
                                <span className="font-bold">EV ADRESİ:</span> <span>{customer.sehir || '.......'} / {customer.ilce || '.......'} <br />{emptyLine}</span>
                                ...
                                <span className="font-bold">İŞ ADRESİ:</span> <span>{val(customer.meslek_is, '')} {emptyLine}</span>
                                <span className="font-bold">İŞ YERİ ÜNVANI:</span> <span>{emptyLine}</span>
                            </div>
                        </td>
                        <td className="border border-black p-2 align-top h-40">
                            <div className="grid grid-cols-[80px_1fr] gap-1">
                                ...
                                <span className="font-bold">İŞ ADRESİ:</span> <span>{val(customer.kefil_meslek_is, '')} {emptyLine}</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <table className="w-full border-collapse border border-black mb-4 text-[9px]">
                ...
                <tbody>
                    <tr>
                        ...
                        <td className="border border-black p-2">{productInfo.name || '...................................................'}</td>
            ...
            {/* --- SIGNATURES --- */}
            <div className="flex justify-between text-xs font-bold text-center mt-8">
                <div className="w-1/3">
                    <p className="mb-8">BORÇLU (ALICI)</p>
                    <p>İMZA</p>
                    <div className="h-16"></div>
                </div>
                <div className="w-1/3">
                    <p className="mb-2">SATICI</p>
                    <p>Ceptekolay Elektronik Ticaret Ltd. Şti.</p>
                    <p className="mt-6">İMZA / KAŞE</p>
                    <div className="h-16"></div>
                </div>
                <div className="w-1/3">
                    <p className="mb-8">MÜŞTEREK BORÇLU (KEFİL)</p>
                    <p>İMZA</p>
                    <div className="h-16"></div>
                </div>
            </div>
